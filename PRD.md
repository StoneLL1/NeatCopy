...
> 文档版本：v1.9.0 | 日期：2026-03-31 | 平台：Windows 11 | 技术栈：Python 3 + PyQt6
...
- [ ] **通用 Tab**：双击Ctrl+C（开关+间隔滑块）、独立热键（开关+按键录制）、开机自启、Toast通知开关、界面主题、轮盘基本设置、预览面板设置
...
| `HotkeyManager` | 全局热键监听（双击Ctrl+C + 独立热键 + 轮盘切换热键 + 预览热键） | Win32 `RegisterHotKey` + `WH_KEYBOARD_LL` |
| `ClipProcessor` | 调度规则引擎或 LLM 客户端，写回剪贴板，发射预览信号 | `win32clipboard` |
...
| `PreviewWindow` | LLM 预览面板（置顶悬浮、毛玻璃、可编辑） | `PyQt6`，Win32 DWM API |
