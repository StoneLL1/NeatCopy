...
```
src/
├── main.py              # 入口，初始化 QApplication + TrayManager + PreviewWindow，轮盘编排
├── tray_manager.py      # QSystemTrayIcon，托盘菜单，图标三态变色，Toast，锁定Prompt显示
├── hotkey_manager.py    # 全局热键（双击Ctrl+C + 独立热键 + 轮盘切换热键 + 预览热键），Win32 API
├── clip_processor.py    # 调度入口：读剪贴板 → 规则或LLM → 写回剪贴板 + 发射预览信号
...
└── ui/
    ├── settings_window.py  # PyQt6 三Tab设置界面（通用/规则/大模型），含轮盘配置分组
