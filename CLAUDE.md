...
```
src/
├── main.py              # 入口，初始化 QApplication + TrayManager
├── tray_manager.py      # QSystemTrayIcon，托盘菜单，图标三态变色，Toast
├── hotkey_manager.py    # 全局热键（双击Ctrl+C + 独立热键），keyboard 库
├── clip_processor.py    # 调度入口：读剪贴板 → 规则或LLM → 写回剪贴板
├── rule_engine.py       # 8条清洗规则，纯正则/算法，无网络
├── llm_client.py        # OpenAI 兼容接口，httpx 异步，30s超时
├── config_manager.py    # 读写 %APPDATA%\NeatCopy\config.json
└── ui/
    └── settings_window.py  # PyQt6 三Tab设置界面
```
...
# 打包为单 exe
pyinstaller --onefile --windowed --name NeatCopy src/main.py
```
